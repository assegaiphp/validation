...
Use this format for `assegaiphp/validation` commits and pull requests.
...
```text
fix(validation): preserve custom rule error messages
feature(validation): add date validation rule
test(validation): cover nested array validation
docs(validation): clarify rule syntax
```
